...

{
  ...
  ObjectList<Object> objList, ObjectPtr object, ObjectList<Object>
  ::IteratorList* iteratorListPtr) 
  {
...
TEST_F(ObjectListTest, IsAnExistingObject) 
{
  ...
  (*DIST_THRESHOLD(objectList)) = 10000000000;
...
TEST_F(ObjectListTest, Add) 
{
  ...
  *(COUNTER_THRES(objectList)) = 4;
  ...
  EXPECT_EQ(object1, *(objectList.begin()));
  //~ EXPECT_EQ(object3 ,*(objectList.end()));
  EXPECT_EQ(1u, object3->getCounter());
  EXPECT_FALSE (object3->getLegit());

  // Add (-0.125,-0,125, 0), deletes (-0.5, 0, 0) object 5 will replace object 1
  EXPECT_FALSE(objectList.add(object5));
  EXPECT_EQ(2u, objectList.size());
  EXPECT_EQ(object3, *objectList.begin());
  //~ EXPECT_EQ(object3,*(--it));
  EXPECT_EQ(2u, object5->getCounter());
  EXPECT_FALSE(object5->getLegit());
    
  // Add (0.125, 0.125, 0), deletes (-0.125,-0,125, 0) and (0.5, 0, 0) object 8 will replace both object 5 and object 3
  EXPECT_FALSE(objectList.add(object8));
  EXPECT_EQ(1u, objectList.size());
  EXPECT_EQ(object8, *objectList.begin());
  EXPECT_EQ(4u, object8->getCounter());
  EXPECT_FALSE( object8->getLegit());

  // Add (0, 0, 0), deletes (0.125, 0.125, 0)  object 2 replaces object 8
  EXPECT_FALSE( objectList.add(object2));
  EXPECT_EQ(1u, objectList.size());
  EXPECT_EQ(object2, *objectList.begin());
